        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="mb-8 flex items-center justify-between">
              ...
              <span
                className={`rounded-full px-3 py-1 text-xs font-bold uppercase ${order.status === "completed" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                {order.status}
              </span>
            </div>
                ...
                <div
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-500">
                      Qty: {item.quantity} × $
                      {parseFloat(item.unitPrice || item.product.price).toFixed(
                        2,
                      )}
                    </p>
                  </div>
                  <p className="text-sm font-bold text-slate-900">
                    ...
                    {(
                      item.quantity *
                      parseFloat(item.unitPrice || item.product.price)
                    ).toFixed(2)}
                  </p>
                </div>
              ))}
            ...
            <div className="mt-8 rounded-xl bg-slate-50 p-4">
              <p className="mb-2 text-xs uppercase tracking-wider text-slate-400 font-semibold">
                Customer & Payment
              </p>
